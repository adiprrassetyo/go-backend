...
func TestTemplateAutoEscapeServer() {
	server := http.Server{
		Addr:    "localhost:8080",
...
func TestTemplateAutoEscapeDisabled() {
	request := httptest.NewRequest(http.MethodGet, "http://localhost:8080", nil)
	recorder := httptest.NewRecorder()
...
func TestTemplateAutoEscapeDisabledServer() {
	server := http.Server{
		Addr:    "localhost:8080",
